...
export class Shop extends Scene {
  private character: { tier: string, color: string; luck: number; sprite: string; name: string };
  private characterNames: string[] = [
    ...
    // Mages
    "Green Mages", "Red Mages", "Pink Mages", "White Mages", "Yellow Mages", "Blue Mages",
    // Warrior Princesses
    ...
    // Sumo Wrestler
    "Red Sumo Wrestler", "Blue Sumo Wrestler", "Green Sumo Wrestler", "White Sumo Wrestler", "Yellow Sumo Wrestler", "Pink Sumo Wrestler",
    // Zombies
    "Green Zombie", "Pink Zombie", "Blue Zombie", "Red Zombie", "Yellow Zombie", "White Zombie",
    // Vikings
    "Yellow Viking", "Green Viking", "Pink Viking", "Blue Viking", "White Viking", "Red Viking",
    // Vaklyries
    "Pink Valkyrie", "Green Valkyrie", "Blue Valkyrie", "Red Valkyrie", "Yellow Valkyrie", "White Valkyrie",
    // Mech Warriors
    "Red Mecha Warrior", "Green Mecha Warrior", "Blue Mecha Warrior", "Pink Mecha Warrior", "Yellow Mecha Warrior", "White Mecha Warrior",
    // Rainbow
    "Rainbow Viking", "Rainbow Valkyrie", "Rainbow Beast Rider", "Rainbow Mage", "Rainbow Ninja", "Rainbow Tribal Chief", "Rainbow Great Elder", "Rainbow Knight", 
  ];
  ...
  create(): void {
    ...
    // Potions
    this.createPotion(centerX - 100, centerY + 210, "Devil's Potion", "devilsPotion", 5000, -0.2, 25, this.buyDevilsPotion.bind(this));
    this.createPotion(centerX + 100, centerY + 210, "Leprechaun's Potion", "leprechaunsPotion", 2000, 2, 2, this.buyLeprechaunsPotion.bind(this));
  ...
  private createPotion(x: number, y: number, name: string, sprite: string, price: number, min: number, max: number, buyPotion: () => void): void {
    const potion = this.add.image(x, y, sprite)
      .setInteractive()
    ...
    this.add.text(x, y + 80, `Price: ${price}`, {
      fontFamily: "Arial",
      fontSize: 20,
      color: "#ffffff",
    }).setOrigin(0.5);
  ...
  private async buyPotion(price: number, buyPotion: () => void, potionName: string, potionSprite: string): Promise<void> {
    try {
  ...
  private showPotionModal(potionName: string, potionSprite: string): void {
    ...
    const modalText = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY - 100, `You purchased ${potionName}!`, {
      fontFamily: "Arial",
      fontSize: 24,
      color: "#000000",
    }).setOrigin(0.5);

    const potionImage = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY - 20, potionSprite)
    .setOrigin(0.5)
    .setDisplaySize(100, 100);

    const luckText = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 60, `Luck Multiplier: ${this.character.luck}`, {
      fontFamily: "Arial",
      fontSize: 20,
      color: "#000000",
    }).setOrigin(0.5);
    
    const closeButton = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 110, "Close", {
      fontFamily: "Arial",
      fontSize: 24,
      color: "#ffffff",
  ...
  private buyDevilsPotion(): void {
    ...
    if (rand < 0.25) {
        ...
        luckBonus = parseFloat((Math.random() * (0.00 + 0.2) - 0.2).toFixed(2));
    } else if (rand < 0.75) {
  ...
  private createDoor(x: number, y: number, tier: "Bronze" | "Silver" | "Gold" | "Rainbow", minLuck: number, maxLuck: number, price: number): void {
    const door = this.add.image(x, y, `${tier}Door`).setInteractive().setDisplaySize(100, 150);
  ...
  private assignCharacter(tier: "Bronze" | "Silver" | "Gold" | "Rainbow", minLuck: number, maxLuck: number): void {
  ...
  private showCharacterModal(): void {
    const modalBackground = this.add.rectangle(this.cameras.main.centerX, this.cameras.main.centerY, 520, 420, 0x000000, 0.8).setOrigin(0.5);
    const modalBox = this.add.rectangle(this.cameras.main.centerX, this.cameras.main.centerY, 500, 400, 0xffffff).setOrigin(0.5);
    const modalText = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY - 150, `Congratulations! You Got ${this.character.name}`, {
      fontFamily: "Arial",
      fontSize: 24,
      color: "#000000",
    }).setOrigin(0.5);

    const characterImage = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY - 50, this.character.sprite)
      .setOrigin(0.5)
      .setDisplaySize(100, 100);

    const characterDetails = this.add.text(this.cameras.main.centerX - 60, this.cameras.main.centerY + 20, 
    ...
    const closeButton = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 150, "Close", {
      fontFamily: "Arial",
      fontSize: 24,
      color: "#ffffff",
      backgroundColor: "#4e342e",
